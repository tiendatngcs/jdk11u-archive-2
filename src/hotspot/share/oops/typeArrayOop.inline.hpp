...
inline jchar*    typeArrayOopDesc::char_base()   { return (jchar*)   base(T_CHAR); }
inline jboolean* typeArrayOopDesc::bool_base()   { return (jboolean*)base(T_BOOLEAN); }
inline jbyte*    typeArrayOopDesc::byte_base()   { return (jbyte*)   base(T_BYTE); }
inline jint*     typeArrayOopDesc::int_base()    { return (jint*)    base(T_INT); }
inline jlong*    typeArrayOopDesc::long_base()   { return (jlong*)   base(T_LONG); }
inline jshort*   typeArrayOopDesc::short_base()  { return (jshort*)  base(T_SHORT); }
inline jfloat*   typeArrayOopDesc::float_base()  { return (jfloat*)  base(T_FLOAT); }
inline jdouble*  typeArrayOopDesc::double_base() { return (jdouble*) base(T_DOUBLE); }

inline jbyte* typeArrayOopDesc::byte_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &byte_base()[which];
}

inline jboolean* typeArrayOopDesc::bool_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &bool_base()[which];
}

inline jchar* typeArrayOopDesc::char_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &char_base()[which];
}

inline jint* typeArrayOopDesc::int_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &int_base()[which];
}

inline jshort* typeArrayOopDesc::short_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &short_base()[which];
}

inline jushort* typeArrayOopDesc::ushort_at_addr(int which) {  // for field descriptor arrays
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return (jushort*) &short_base()[which];
}

inline jlong* typeArrayOopDesc::long_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &long_base()[which];
}

inline jfloat* typeArrayOopDesc::float_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
  return &float_base()[which];
}

inline jdouble* typeArrayOopDesc::double_at_addr(int which) {
  increase_access_counter();
  assert(is_within_bounds(which), "index %d out of bounds %d", which, length());
